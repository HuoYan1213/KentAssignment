...
// Function to get the next available photo number
function get_next_photo_number($folder) {
    ...
    foreach ($files as $file) {
        ...
        $number = intval($basename); // Will extract the number part
        $numbers[] = $number;
    }
...
// Login user
function login($user, $url = 'main.php') {
    $_SESSION['user'] = $user;
    redirect($url);
}

// Logout user
function logout($url = 'main.php') {
    unset($_SESSION['user']);
    redirect($url);
}
...
function get_mail() {
    ...
    $m->Port = 587;
    $m->Username = 'joeylaumh06@gmail.com';
    $m->Password = 'aihp eodw neas lwqj';
    $m->CharSet = 'utf-8';
...
// Global PDO object - loaded from db.php
require_once 'db.php';
